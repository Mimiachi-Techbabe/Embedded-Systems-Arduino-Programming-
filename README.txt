...
In this project, We are using push Buttons to start the count on the 7-Segment, and only the number 9 is used. Once the Middle btton on the board is pressed, it counts 9, and the Buzzer turns on. Then,once the upper Button is pushed, the 7-segment is turned off with the Buzzer, then the oled Screen prints "WIN WIN".
...
To set it up, in your void setup you have to select a baud rate which is basically the rate at which the data will be transferred and that has to match the rate set in the serial monitor window.
